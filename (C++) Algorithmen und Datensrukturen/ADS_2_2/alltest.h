...
inline void check_memory(int acall, int scall) {
    if (acall < 0 || scall < 0) {
        throw std::runtime_error("check_memory, cannot happen");
    }
    check_memory(static_cast<size_t>(acall), 
                 static_cast<size_t>(scall));
}
...
protected:
    virtual void SetUp() {
        ...
        scall = static_cast<int>(scurrent_alloc);
        acall = static_cast<int>(acurrent_alloc);
        sfreed = static_cast<int>(scurrent_freed);
        afreed = static_cast<int>(acurrent_freed);
