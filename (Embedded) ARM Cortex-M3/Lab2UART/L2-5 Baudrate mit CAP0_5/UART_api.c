...
float erg;
int trueErg;
float baud = 0xFFFFFFFF;
float firstInterrupt = 0;
float secondInterrupt = 0;
...
void TIMER2_IRQHandler (void){	// Capture measurement of baudrate
	...
	if (firstInterrupt != 0 && secondInterrupt == 0){
		secondInterrupt = LPC_TIM2->TC;
		if(secondInterrupt - firstInterrupt < baud){
			baud = secondInterrupt - firstInterrupt;
			erg = 25000000/baud;
		}
		firstInterrupt = 0;
		secondInterrupt = 0;
...
*****************************************************************************/
void init_Timer2(void){
	LPC_PINCON->PINSEL0 = 0xC00;				// Setzt P0.5 auf Eingang
	//LPC_GPIO0->FIODIR &= ~(1 << 5);		// Setzt P0.5 auf Eingang
	
	LPC_SC->PCONP |= 1<<22;							// TIMER2 aktivieren
	LPC_TIM2->CCR = 0x38;								// (0b0011 1000) Interrupt bei steigender und fallender Flanke bei CAP1
	
	LPC_TIM2->PR = 25;

	LPC_TIM2->MCR = 0x0;								// Alle Match Register ausschalten
	LPC_TIM2->TCR = 0x2;          			// Reset Timer
	LPC_TIM2->TCR = 0x1;          			// Start Timer 2
...
void init_NVIC(void){		// Interrupt enable
	...
	NVIC_SetPriority(TIMER2_IRQn, 2);  // TIMER2 Prio auf 3
