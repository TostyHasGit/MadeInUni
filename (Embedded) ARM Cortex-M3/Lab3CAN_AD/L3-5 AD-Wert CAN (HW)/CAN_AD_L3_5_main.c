...
const uint16_t My_CAN_ID = 0x707;
...
int main(void)
{
	...
	while(1)
	{
				if(AD_New_Value_Ready)
				{
					...
					Delta_Voltage = Old_AD_Voltage - AD_Voltage;
					if (Delta_Voltage > 0.5 || Delta_Voltage < (-0.5)){
						Old_AD_Voltage = AD_Voltage;
						sprintf(AD_Val_String, "%.3fV", AD_Voltage);
						GLCD_DisplayString(2, 14, (unsigned char*)AD_Val_String);
						
						My_CAN_TX_Message.length = strlen(AD_Val_String);
						
						for (j = 0; j < My_CAN_TX_Message.length; ++j){
							My_CAN_TX_Message.data[j] = AD_Val_String[j];
						}
						
						CAN_Send_Message(1, &My_CAN_TX_Message);
					}
					
					AD_New_Value_Ready = 0;				// Reset
				}	
		...
		GLCD_SetTextColor(Blue);                        /* Set the Text Color */ 	
				...
				if(CAN1_New_RX_Data_Stored)
				{
					...
					// - Convert the received ID value to a string called ID_String
					CAN_Receive_Message(1, &My_CAN1_RX_message);
					sprintf(ID_String, "%x", My_CAN1_RX_message.id);
					// - Collect the received and convert them to the string AD_Val_String
					for (j = 0; j < My_CAN1_RX_message.length; j++){
						AD_Val_String[j] = My_CAN1_RX_message.data[j];
					}
					...
					GLCD_DisplayString(i, 12, (unsigned char*) ID_String);/* print string to LCD */
					GLCD_DisplayString(i+1, 14, (unsigned char*) AD_Val_String);/* print string to LCD */
...
{  
 ...
 //Read and store AD value
		...
		AD_Voltage = (AD_Value >> 4);
		AD_Voltage = (AD_Voltage * 3.3) / 0xFFF;
