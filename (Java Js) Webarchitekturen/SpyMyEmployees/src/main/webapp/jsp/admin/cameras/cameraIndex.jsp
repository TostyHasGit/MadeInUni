			                ...
			                <c:forEach var="location" items="${locations}">
			                    <div class="location-card d-flex justify-content-between align-items-center">
			                        ...
			                        <div class="d-flex gap-2">
			                            <a class="btn btn-outline-light btn-sm" data-bs-toggle="collapse" href="#collapse-${location.id}">
			                                Kameras anzeigen
			                            </a>
			                            <a class="btn btn-outline-light btn-sm"
			                               href="${pageContext.request.contextPath}/admin/camera/create?locationId=${location.id}">
			                        ...
			                        <div class="ps-3 pe-3">
			                            <c:choose>
			                                <c:when test="${empty camerasByLocation[location.id]}">
			                                    <div class="text-muted fst-italic mt-2">Keine Kameras vorhanden.</div>
			                                </c:when>
			                                <c:otherwise>
			                                    <div class="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-6 row-cols-xl-9 g-3">
			                                        <c:forEach var="camera" items="${camerasByLocation[location.id]}">
			                                            <div class="col">
			                                                <a href="${pageContext.request.contextPath}/admin/camera/edit?id=${camera.id}" class="camera-link">
			                                                    <div class="camera-card text-center
                                                                    <c:choose>
                                                                        <c:when test='${cameraHasUncheckedErrors[camera.id]}'>
                                                                            bg-warning text-dark
                                                                        </c:when>
                                                                        <c:when test='${camera.status != null and camera.status.trim().toLowerCase() eq "active"}'>
